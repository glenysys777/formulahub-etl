...
| **Run Pipeline** | `run_pipeline` | **Master → Child** orchestration: run another pipeline with Job Context inherit / child_active / override, `publish_as` for `${child.<name>.…}`, cycle + depth guards. See [MASTER_CHILD_PIPELINES.md](../architecture/MASTER_CHILD_PIPELINES.md). DEMO-safe nested runs |
...
| Joblets, shared contexts, enterprise lineage UI | **Job Contexts** (`metadata.contexts` + `${…}`) shipped. **Run Pipeline** (`run_pipeline`) Master → Child v1 shipped — see [MASTER_CHILD_PIPELINES.md](../architecture/MASTER_CHILD_PIPELINES.md). Enterprise lineage UI still *(planned)* |
...
4. **Master → Child** — `demos/master-file-to-databricks` (Run Pipeline ×2 with QA context inherit; DEMO sidecars)
5. **Lookup + Variables** — `demos/lookup-join-mapper` — two CSV sources → Lookup Join → Field Mapper (Variables) → File
6. **Core path** — Excel → Field Mapper → Filter → Sort → Aggregate → File
7. `demos/excel-to-file` — Excel → Schema Map → Transform → File  
8. `demos/python-row-flex` — API → map → Python Row → File  
9. `demos/s3-pgp-snowflake` — S3 → PGP → Validate → Snowflake demo → Archive  
10. Existing Excel, SFTP, Postgres, API demos under `demos/`

## Open-core honesty

- **OSS core stays:** visual canvas, runner, Field Mapper Variables, Lookup Join, Kafka Source, Databricks Job + Databricks SQL orchestration, Job Contexts / `${…}`, **Run Pipeline** (Master → Child), Community scheduler, AI Build shortcut  
- **Enterprise locks later *(planned)*:** SSO, RBAC, lineage UI, HA scheduler — no fabricated prices  
- Icons are **original SVG/CSS** with text labels (S3 Source, Kafka Source, Databricks Job, Run Pipeline) — never official AWS / Kafka / Databricks trademark logo assets
