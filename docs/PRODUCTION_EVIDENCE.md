...
| C8 | Databricks Free Edition Jobs smoke (Jobs API `run-now` + poll via FormulaETL) | **PROVEN Soft-PASS LIVE_EXTERNAL Jobs only** | `formulaetl.components.databricks_job.DatabricksJob.run`; `FORMULAETL_DEMO=0`; Free Edition Jobs API; PAT **not** stored in repo | Redacted: [`docs/evidence/databricks_job_smoke_redacted.json`](./evidence/databricks_job_smoke_redacted.json) (`job_id`/`run_id`/`TERMINATED`/`SUCCESS` ~21s; notebook `/Shared/formulahub_etl_smoke`). **Does not** prove Spark-inside-FormulaETL or full LIVE wedge. After merge onto `main`, record merge SHA here. | this PR — **replace with merge SHA on main** | 2026-09-14 |
