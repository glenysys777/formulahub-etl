...
1. Create a connection (`sftp` / `s3` / `snowflake` / `postgres` / `http` / `databricks`) with secrets as env refs or local-store values.
...
5. Job Contexts + `${…}` for Databricks SQL/Job: see **[`docs/CONTEXTS.md`](../CONTEXTS.md)**.
6. Full live path (S3|SFTP → PGP → CSV → validate → map → Postgres|Snowflake → archive): see **[`LIVE_WEDGE.md`](./LIVE_WEDGE.md)** (`RUN_LIVE_WEDGE=1`, classification `LIVE_CLOUD`, default **UNPROVEN**).
