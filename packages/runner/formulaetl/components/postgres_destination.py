"""Postgres Destination — insert rows; demo writes SQLite + CSV under data/out/postgres_demo/."""
...
def _write_sqlite(path: Path, table: str, clean: list[dict[str, Any]], if_exists: str) -> None:
    ...
    try:
        ...
        if exists and if_exists == "fail":
            raise ValueError(f"PostgresDestination: table {table!r} already exists")
...
@register
class PostgresDestination(BaseComponent):
    component_type = "postgres_destination"
    ...
    config_schema = {
        ...
        "properties": {
            ...
            "demo_output_dir": {"type": "string", "default": "data/out/postgres_demo"},
        },
    }
    parameters = [
        connection_id_param(),
        {
            ...
            "type": "string",
            "required": False,
            "help": "Postgres DSN (demo mode ignores and writes local SQLite/CSV)",
        },
        {
            "key": "host",
            "label": "Host",
            ...
            "required": False,
            "default": "demo",
            "help": "Postgres host (demo → local mirror)",
        },
        {
            "key": "port",
            "label": "Port",
        ...
        },
        {
            ...
            "type": "string",
            "required": False,
            "default": "data/out/postgres_demo",
    ...
    def run(self, ctx: RunContext, rows: list[dict[str, Any]] | None = None) -> ComponentResult:
        ...
        with timed(metrics):
            ...
            if use_demo:
                out_dir = ctx.resolve(
                    str(self.config.get("demo_output_dir") or "data/out/postgres_demo")
                )
                ...
                note = (
                    "Demo mock: wrote SQLite + CSV under data/out/postgres_demo/ "
                    "(not a live Postgres INSERT). Real load needs DSN + FORMULAETL_DEMO=0."
                )
                ctx.emit(
                    f"PostgresDestination [demo]: {len(clean)} rows → {db_path} [{table}] + {csv_path.name}"
                )
            ...
            if not clean:
                ...
                return ComponentResult(
                    rows=rows,
                    metrics=metrics,
                    side_effects={"mode": "postgres", "rows_loaded": 0, "table": table},
                )
            ...
            metrics.rows_in = len(rows)
            metrics.rows_out = len(rows)
            ctx.emit(f"PostgresDestination: inserted {len(clean)} rows → {table}")
            return ComponentResult(
                rows=rows,
                metrics=metrics,
