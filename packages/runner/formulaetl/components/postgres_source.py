"""Postgres Source — run SQL via psycopg; demo falls back to SQLite / fixture rows."""
...
@register
class PostgresSource(BaseComponent):
    component_type = "postgres_source"
    ...
    parameters = [
        connection_id_param(),
        {
            ...
            "type": "string",
            "required": False,
            "help": "Postgres DSN (or leave empty / host=demo for SQLite demo fallback)",
        },
        {
            "key": "host",
            "label": "Host",
            ...
            "required": False,
            "default": "demo",
            "help": "Postgres host (demo → local SQLite fallback)",
        },
        {
            "key": "port",
            "label": "Port",
    ...
    def run(self, ctx: RunContext, rows: list[dict[str, Any]] | None = None) -> ComponentResult:
        ...
        with timed(metrics):
            ...
            if use_demo:
                ...
                mode = "demo_sqlite"
                note = (
                    "Demo fallback: reading SQLite (not live Postgres). "
                    "Real Postgres needs a DSN/credentials and FORMULAETL_DEMO=0."
                )
                try:
                    if sqlite_path.exists():
                        ...
                        if csv_path.exists() and "orders" in query.lower():
                            ...
                            note = (
                                ...
                                "Not a live Postgres connection."
                            )
                ...
                ctx.emit(f"PostgresSource [{mode}]: → {len(out_rows)} rows ({note[:60]}…)")
            ...
            metrics.rows_out = len(out_rows)
            ctx.emit(f"PostgresSource: query → {len(out_rows)} rows")
            return ComponentResult(
                rows=out_rows,
                metrics=metrics,
                artifacts={"query": query},
                side_effects={"mode": "postgres", "rows": len(out_rows)},
            )
